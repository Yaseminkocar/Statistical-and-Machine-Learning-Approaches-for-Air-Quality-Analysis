...
if pm_cols:
    ...
    plt.ylabel("µg/m³")
    plt.savefig("boxplots_pm.pdf", format='pdf', bbox_inches="tight")
    plt.close()
else:
...
if col_temp is not None:
    ...
    plt.savefig("histogram_temperature.pdf", format='pdf', bbox_inches="tight")
    plt.close()
else:
...
if (col_temp is not None) and (col_pm25 is not None):
    ...
    plt.xlabel("Temperature (°C)")
    plt.ylabel("PM2.5 (µg/m³)")
    plt.savefig("scatter_pm_temp.pdf", format='pdf', bbox_inches="tight")
    plt.close()
else:
...
if col_voc is not None and df[col_voc].notna().any():
    ...
    plt.xlabel("VOC")
    plt.ylabel("Count")
    plt.savefig("bar_voc.pdf", format='pdf', bbox_inches="tight")
    plt.close()
    ...
    plt.xlabel("Device")
    plt.ylabel("Count")
    plt.savefig("bar_device.pdf", format='pdf', bbox_inches="tight")
    plt.close()
else:
    print("!! Warning: Neither VOC nor device column found; bar chart skipped.")


print(" - boxplots_pm.pdf")
print(" - histogram_temperature.pdf")
print(" - lineplot_pm25.pdf")
print(" - scatter_pm_temp.pdf")
print(" - bar_voc.pdf or bar_device.pdf")